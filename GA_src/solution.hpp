...
private:
    ...
    {
        version = ver;
        id = address + std::to_string(version);
    }
    int sum_edges(void);
...
public:
    ...
    {
        Gene R(num_edges);
        bit::transform(all_of(gene), R.begin(), OPER_FLIP);
        return R;
    }
    ...
    std::string gene_to_string()
    {
        std::string gene_string;
        for (const auto &bit : gene)
        {
            gene_string += bit ? "1" : "0";
        }
        return gene_string;
...
{
    if (Graph::get_instance_owner() == id)
        return;
...
Solution &Solution::make_span_wide(Real r_drop = 0)
{
    ...
    for (int u = 1; u <= num_nodes; u++)
        if (is_node_involved[u] && !is_terminal[u])
            nodes.push_back(u);
...
std::ostream &operator<<(std::ostream &stream, Solution solution)
{
    ...
    else if (IS_MEDIUM_INSTANCE(num_edges))
    {
